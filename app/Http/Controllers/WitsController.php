...
class WitsController extends Controller
{
    ...
    return view('wits.index', compact('data', 'memberGroups'));
    }
    ...
    public function create()
    {
        ...
        return view('wits.create', compact( 'data', 'memberGroups'));
    }
    ...
    $capetown->save();
        ...
        return redirect(route('wits.index'));
    }

    public function edit($id)
    {
        $data = Wits::findOrFail($id);
        $memberGroups = MemberGroup::all();
         

        //dd($data);
        return view('wits.edit',compact('data','memberGroups' ));
    }
    ...
    public function sendInvoices(Request $request)
{
    ...
    $validated = $request->validate([
        ...
        'selected_members.*' => 'exists:wits,id',
    ]);

    $members = Wits::whereIn('id', $validated['selected_members'])->get();
        ...
        if ($member->email) {
            try {
                ...
                if (!$member->invoice_number) {
                    ...
                    $latestInvoice = Wits::where('invoice_number', 'LIKE', "{$yearMonth}-%")
