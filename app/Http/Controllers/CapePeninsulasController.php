...
class CapePeninsulasController extends Controller
{
    public function index(Request $request)
    {
    $query = CapePeninsula::query();
    ...
    return view('capepen.index', compact('data', 'memberGroups'));
    }

    public function create()
    {
        $data = CapePeninsula::all();
        $memberGroups = MemberGroup::all();

        return view('capepen.create', compact( 'data', 'memberGroups'));
    }
    ...
    public function store(CapePenRequest $request)
    {
        ...
        $capePeninsula = CapePeninsula::create($data);

         // Associate with the selected member group
    $capePeninsula->member_group_id = $request->member_group_id;
    $capePeninsula->save();

        toastr()->success('Member Saved');

        return redirect(route('capepen.index'));
    }

    public function edit($id)
    {
        $data = CapePeninsula::findOrFail($id);
        $memberGroups = MemberGroup::all();
        
         

        //dd($data);
        return view('capepen.edit',compact('data','memberGroups' ));
    }
    ...
    {
        $data = $request->all();
        $record = CapePeninsula::findOrFail($id);
    ...
    public function sendInvoices(Request $request)
{
    ...
    $validated = $request->validate([
        ...
        'selected_members.*' => 'exists:cape_peninsulas,id',
    ]);

    $members = CapePeninsula::whereIn('id', $validated['selected_members'])->get();
        ...
        if ($member->email) {
            try {
                ...
                if (!$member->invoice_number) {
                    ...
                    $latestInvoice = CapePeninsula::where('invoice_number', 'LIKE', "{$yearMonth}-%")
