...
class YoungProfessionalController extends Controller
{
    public function index(Request $request)
    {
    $query = YoungProfessional::query();
    ...
    return view('youngprofessional.index', compact('data', 'memberGroups'));
    }

    public function create()
    {
        $data = YoungProfessional::all();
        $memberGroups = MemberGroup::all();

        return view('youngprofessional.create', compact( 'data', 'memberGroups'));
    }
    ...
    public function store(MemberCrudRequest $request)
    {
        ...
        $members = YoungProfessional::create($data);
    ...
    $members->save();
        ...
        return redirect(route('youngprofessional.index'));
    }

    public function edit($id)
    {
        $data = YoungProfessional::findOrFail($id);
        $memberGroups = MemberGroup::all();
        
         
        //dd($data);
        return view('youngprofessional.edit',compact('data','memberGroups'));
    }
    ...
    {
        $data = $request->all();
        $record = YoungProfessional::findOrFail($id);
    ...
    public function sendInvoices(Request $request)
{
    ...
    $validated = $request->validate([
        ...
        'selected_members.*' => 'exists:young_professionals,id',
    ]);

    $members = YoungProfessional::whereIn('id', $validated['selected_members'])->get();
        ...
        if ($member->invoice_email) {
            try {
                ...
                if (!$member->invoice_number) {
                    ...
                    $latestInvoice = YoungProfessional::where('invoice_number', 'LIKE', "{$yearMonth}-%")
