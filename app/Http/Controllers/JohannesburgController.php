...
class JohannesburgController extends Controller
{
    public function index(Request $request)
{
    $query = Johannesburg::query();
    ...
    return view('johannesburg.index', compact('data', 'memberGroups'));
}

    public function create()
    {
        $data = Johannesburg::all();
        $memberGroups = MemberGroup::all();

        return view('johannesburg.create', compact( 'data', 'memberGroups'));
    }
    ...
    public function store(CapePenRequest $request)
    {
        ...
        $johannesburg = Johannesburg::create($data);

         // Associate with the selected member group
    $johannesburg->member_group_id = $request->member_group_id;
    $johannesburg->save();

        toastr()->success('Member Saved');

        return redirect(route('johannesburg.index'));
    }

    public function edit($id)
    {
        $data = Johannesburg::findOrFail($id);
        $memberGroups = MemberGroup::all();
        

        //dd($data);
        return view('johannesburg.edit',compact('data','memberGroups' ));
    }
    ...
    {
        $data = $request->all();
        $record = Johannesburg::findOrFail($id);
    ...
    public function sendInvoices(Request $request)
{
    ...
    $validated = $request->validate([
        ...
        'selected_members.*' => 'exists:johannesburgs,id',
    ]);

    $members = Johannesburg::whereIn('id', $validated['selected_members'])->get();
        ...
        if ($member->email) {
            try {
                ...
                if (!$member->invoice_number) {
                    ...
                    $latestInvoice = Johannesburg::where('invoice_number', 'LIKE', "{$yearMonth}-%")
