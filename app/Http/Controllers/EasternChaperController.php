...
class EasternChaperController extends Controller
{
    public function index(Request $request)
    {
    $query = EasternChapter::query();
    ...
    return view('easternchap.index', compact('data', 'memberGroups'));
    }

    public function create()
    {
        $data = EasternChapter::all();
        $memberGroups = MemberGroup::all();

        return view('easternchap.create', compact( 'data', 'memberGroups'));
    }
    ...
    public function store(MemberCrudRequest $request)
    {
        ...
        $members = EasternChapter::create($data);
    ...
    $members->save();
        ...
        return redirect(route('easternchap.index'));
    }

    public function edit($id)
    {
        $data = EasternChapter::findOrFail($id);
        $memberGroups = MemberGroup::all();
        

        //dd($data);
        return view('easternchap.edit',compact('data','memberGroups'));
    }

    public function update(MemberCrudRequest $request, $id)
    {
        $data = $request->validated();
        $record = EasternChapter::findOrFail($id);
    ...
    public function sendInvoices(Request $request)
{
    ...
    $validated = $request->validate([
        ...
        'selected_members.*' => 'exists:eastern_chapters,id',
    ]);

    $members = EasternChapter::whereIn('id', $validated['selected_members'])->get();
        ...
        if ($member->invoice_email) {
            try {
                ...
                if (!$member->invoice_number) {
                    ...
                    $latestInvoice = EasternChapter::where('invoice_number', 'LIKE', "{$yearMonth}-%")
