...
class PortElizabethController extends Controller
{
    public function index(Request $request)
{
    $query = PortElizabeth::query();
    ...
    return view('upe.index', compact('data', 'memberGroups'));
}

    public function create()
    {
        $data = PortElizabeth::all();
        $memberGroups = MemberGroup::all();

        return view('upe.create', compact( 'data', 'memberGroups'));
    }
    ...
    public function store(CapePenRequest $request)
    {
        ...
        $capetown = PortElizabeth::create($data);
    ...
    $capetown->save();
        ...
        return redirect(route('upe.index'));
    }

    public function edit($id)
    {
        $data = PortElizabeth::findOrFail($id);
        $memberGroups = MemberGroup::all();
        

        //dd($data);
        return view('upe.edit',compact('data','memberGroups' ));
    }
    ...
    {
        $data = $request->all();
        $record = PortElizabeth::findOrFail($id);
    ...
    public function sendInvoices(Request $request)
{
    ...
    $validated = $request->validate([
        ...
        'selected_members.*' => 'exists:port_elizabeths,id',
    ]);

    $members = PortElizabeth::whereIn('id', $validated['selected_members'])->get();
        ...
        if ($member->email) {
            try {
                ...
                if (!$member->invoice_number) {
                    ...
                    $latestInvoice = PortElizabeth::where('invoice_number', 'LIKE', "{$yearMonth}-%")
