...
class KznChapterController extends Controller
{
    public function index(Request $request)
{
    $query = KznChapter::query();
    ...
    return view('kznchap.index', compact('data', 'memberGroups'));
}
    ...
    public function create()
    {
        ...
        return view('kznchap.create', compact( 'data', 'memberGroups'));
    }
    ...
    public function store(MemberCrudRequest $request)
    {
        ...
        $members = KznChapter::create($data);
    ...
    $members->save();
        ...
        return redirect(route('kznchap.index'));
    }

    public function edit($id)
    {
        $data = KznChapter::findOrFail($id);
        $memberGroups = MemberGroup::all();

        //dd($data);
        return view('kznchap.edit',compact('data','memberGroups' ));
    }
    ...
    {
        $data = $request->validated();
        $record = KznChapter::findOrFail($id);
    ...
    public function sendInvoices(Request $request)
{
    ...
    $validated = $request->validate([
        ...
        'selected_members.*' => 'exists:kzn_chapters,id',
    ]);

    $members = KznChapter::whereIn('id', $validated['selected_members'])->get();
        ...
        if ($member->invoice_email) {
            try {
                ...
                if (!$member->invoice_number) {
                    ...
                    $latestInvoice = KznChapter::where('invoice_number', 'LIKE', "{$yearMonth}-%")
